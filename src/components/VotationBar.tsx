...
const VotationBar: FC<VotationBar> = ({ votes }) => {
  ...
  const getPositiveResize = () => {
    const totalVotes = positive + negative;
    const percent = (positive * 100) / totalVotes;
    return Math.ceil(percent);
  }

  const getNegativeResize = () => {
    const totalVotes = positive + negative;
    const percent = (negative * 100) / totalVotes;
    return Math.ceil(percent);
  }
  ...
  return (
    <div className="votation__bar absolute bottom-0 flex justify-between w-full h-20 md:h-12">
      <div className={`votation__possitive w-[${getPositiveResize()}%] h-full bg-dark-teal flex items-center justify-start px-6 md:px-4`}>
      ...
      <div className={`votation__negative  w-[${getNegativeResize()}%] h-full bg-dark-yellow flex items-center justify-end px-6 md:px-4`}>
